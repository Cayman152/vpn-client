...
public class CheckUpdateViewModel : MyReactiveObject
{
    ...
    private readonly string _GhostVPN = ECoreType.GhostVPN.ToString();
    ...
    public CheckUpdateViewModel(Func<EViewAction, object?, Task<bool>>? updateView)
    {
        ...
        CheckUpdateCmd.ThrownExceptions.Subscribe(ex =>
        {
            ...
            _ = UpdateView(_GhostVPN, ex.Message);
        });
    ...
    private void RefreshCheckUpdateItems()
    {
        ...
        {
            CheckUpdateModels.Add(GetCheckUpdateModel(_GhostVPN));
    ...
    {
        if (coreType == _GhostVPN && Utils.IsPackagedInstall())
        {
            return new()
            {
                IsSelected = false,
    ...
    private async Task CheckUpdateTask()
    {
        ...
        for (var k = CheckUpdateModels.Count - 1; k >= 0; k--)
        {
            ...
            else if (item.CoreType == _GhostVPN)
            {
                if (Utils.IsPackagedInstall())
                {
                    await UpdateView(_GhostVPN, "Not Support");
                    continue;
                }
    ...
    private async Task CheckUpdateN(bool preRelease)
    {
        async Task _updateUI(bool success, string msg)
        {
            await UpdateView(_GhostVPN, msg);
            if (success)
            {
                await UpdateView(_GhostVPN, ResUI.OperationSuccess);
                UpdatedPlusPlus(_GhostVPN, msg);
            }
        }
        await new UpdateService(_config, _updateUI).CheckUpdateGuiN(preRelease)
            .ContinueWith(t => UpdatedPlusPlus(_GhostVPN, ""));
    }
    ...
    private async Task UpdateFinished()
    {
        if (_lstUpdated.Count > 0 && _lstUpdated.Count(x => x.IsFinished == true) == _lstUpdated.Count)
        {
            ...
            if (_lstUpdated.Any(x => x.CoreType == _GhostVPN && x.IsFinished == true))
            {
    ...
    private async Task UpgradeN()
    {
        try
        {
            var fileName = _lstUpdated.FirstOrDefault(x => x.CoreType == _GhostVPN)?.FileName;
            if (fileName.IsNullOrEmpty())
            {
                return;
            }
            ...
            {
                await UpdateView(_GhostVPN, ResUI.UpgradeAppNotExistTip);
        ...
        {
            await UpdateView(_GhostVPN, ex.Message);
        }
    }
